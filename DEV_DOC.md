...
Even if the containers are destroyed, the data remains intact on the host machine inside the `/home/slimane/data/` directory until manually removed or deleted via `make down` (which removes the volume declarations in Docker, and depending on your Makefile, might clear the local folders).
